...
/**
 * /welcome — first run, straight after sign-up. Epic 9.13, Part D.
 *
 ...
 * mostly copy and a three-value state machine — and why a change to the intake
 * flow later cannot silently diverge between here and Compare.
 *
 ...
 *
 * It is mechanically the same scan Epic 9.8 ran against psmdigitalagency.com.
 * Nothing here is special-cased, which is also why the resulting client is an
 * ORDINARY row on the dashboard — see the note on `Done` below.
 *
        ...
        {step.kind === 'result' ? (
          ...
          <div className="flex flex-col gap-6">
            <Card elevation="seated">
              <CardBody>
                <IntakeForm
                  onStarted={() => setStep({ kind: 'working' })}
                  onClassified={(client) => setStep({ kind: 'result', client })}
                  onFailed={() => setStep({ kind: 'ask' })}
                />
              </CardBody>
            </Card>
